# SpartaCSS

SpartaCSS is RedSpartan Systems' independent, reusable design system — design
tokens, a base reset/layout layer, a core set of UI components, an icon
system, and a notifications feature module.

**Status:** core, icon system, and notifications module all extracted. The
package is not yet installable — no `package.json` or build tooling exists
yet. See `docs/adr/0001-package-architecture.md` for the architecture this
repository is being built against, and `docs/extraction-plan.md` for
extraction status and inventory detail.

## License

Apache License 2.0 — see [LICENSE](./LICENSE). RedSpartan Labs branding,
trademarks, and project identity are separate from the license grant.
